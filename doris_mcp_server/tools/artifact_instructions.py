...
class ArtifactInstructionsTool:
    """Artifact Instructions Tool Implementation"""
...
"""
    ...
    ARTIFACT_INSTRUCTIONS_HEAD = """<artifact_instructions>
...
After obtaining the instructions, use them appropriately and avoid duplicate calls for the same type.
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "type": {"type": "string", "enum": ['code', 'documents', 'html', 'svg', 'mermaid', 'react'], "description": "Type of artifact to get instructions for"}
                },
                "required": ["type"]
            })
    
    async def execute(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Execute the tool and return artifact instructions"""
        artifact_type = arguments.get("type")  
        
        # Prepare the response
        response = {}
        # 根据artifact_type，从EXAMPLE_TEMPLATES中获取对应的模板
        if artifact_type and artifact_type in self.EXAMPLE_TEMPLATES:
            template = self.EXAMPLE_TEMPLATES[artifact_type]
            # Replace template variables with actual values
            template = template.replace("${ARTIFACT_INSTRUCTIONS_HEAD}", self.ARTIFACT_INSTRUCTIONS_HEAD)
            template = template.replace("${ARTIFACT_INSTRUCTIONS_TAIL}", self.ARTIFACT_INSTRUCTIONS_TAIL)
            ...
            response = {
                ...
                {
                    "type": 'text',
                    "text": f'[IMPORTANT NOTE: You have received instructions for artifact type {artifact_type}. DO NOT call this again for the same type.]\n\n{self.ARTIFACTS_INFO}\n\n{template}'
                }
                ]
            }
        return response