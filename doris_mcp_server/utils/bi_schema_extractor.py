...
class MetadataExtractor:
    ...
    async def get_bi_table_schema_async(self, table_name: str, db_name: str = None, catalog_name: str = None) -> List[Dict[str, Any]]:
    ...
    async def exec_query_for_mcp(
        ...
        try:
            ...
            logger.debug(f"Using effective parameters - DB: {effective_db}, Catalog: {effective_catalog}")
            ...
            final_sql = sql
            if effective_catalog or effective_db:
                context_statements = []
                ...
                if effective_catalog:
                    try:
                        ...
                        return self._format_response(
                            success=False, 
                            error=f"Invalid catalog name: {effective_catalog}", 
                ...
                if effective_db:
                    try:
                        ...
                        return self._format_response(
                            success=False, 
                            error=f"Invalid database name: {effective_db}", 
    ...
    async def get_db_table_list_for_mcp(
        ...
        try:
            tables = await self.get_bi_database_tables_async(db_name=effective_db, catalog_name=effective_catalog)
    ...
    async def get_db_list_for_mcp(self, catalog_name: str = None) -> Dict[str, Any]:
        ...
        logger.debug(f"Using effective catalog: {effective_catalog}")
        
        try:
            databases = await self.get_all_databases_async(catalog_name=effective_catalog)
